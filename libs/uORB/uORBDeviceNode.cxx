...
int uORB::DeviceNode::open(FAR struct file *filep)
{
	/* is this a publisher? */
...
int uORB::DeviceNode::close(FAR struct file *filep)
{
...
ssize_t uORB::DeviceNode::read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
...
ssize_t uORB::DeviceNode::write(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	/*
...
int uORB::DeviceNode::ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	switch (cmd) {
	case ORBIOCUPDATED: {
			ATOMIC_ENTER;
...
{
	uORB::DeviceNode *uORBDev = (uORB::DeviceNode *)(filep->f_inode->i_private);
